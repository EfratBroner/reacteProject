...
const AddHelpRequest: FC<AddHelpRequestProps> = ({onSuccess }) => {
  ...
  return (
    <div className="AddHelpRequest">
      ...
      {!isAdd && (<button onClick={() => setIsAdd(true)} className="open-form-btn">הוספת בקשת עזרה</button>)}

      {isAdd && (<form onSubmit={formik.handleSubmit} className="request-form">
        ...
        <div className="form-group">
          ...
          <select name="priority" value={formik.values.priority} onChange={formik.handleChange}>
            {priorityOption.map((opt, index) => (
              <option key={index} value={opt}>{opt}</option>))}</select>
        </div>

        <div className="form-actions">
          <button type="submit" className="submit-btn">שמור בקשה</button>
          <button type="button" className="cancel-btn" onClick={() => { setIsAdd(false); formik.resetForm(); }}>ביטול</button>
        </div>
      </form>
      )}
    </div>
